...
groups() ->
    [
        {test_grp_0_register, [{repeat_until_all_ok, 2}], [
            case01_register,
        ...
        {test_grp_1_read, [{repeat_until_all_ok, 2}], [
            case10_read,
        ...
        {test_grp_2_write, [{repeat_until_all_ok, 2}], [
            case20_write,
            case21_write_object,
            case22_write_error,
            case20_single_write,
            case20_single_write_4_level_path
        ]},
        {test_grp_3_execute, [{repeat_until_all_ok, 2}], [
            case30_execute, case31_execute_error
        ]},
        {test_grp_4_discover, [{repeat_until_all_ok, 2}], [
            case40_discover
        ]},
        {test_grp_5_write_attr, [{repeat_until_all_ok, 2}], [
            case50_write_attribute
        ]},
        {test_grp_6_observe, [{repeat_until_all_ok, 2}], [
            case60_observe
        ]},
        {test_grp_7_block_wize_transfer, [{repeat_until_all_ok, 2}], [
            case70_read_large, case70_write_large
        ]},
        {test_grp_8_object_19, [{repeat_until_all_ok, 2}], [
        ...
        {test_grp_9_psm_queue_mode, [{repeat_until_all_ok, 2}], [
            case90_psm_mode,
            case90_queue_mode
        ]}
    ].
...
case01_register_report(Config) ->
    ...
    MsgId = 12,
    SubTopic = list_to_binary("lwm2m/"++Epn++"/dn/#"),
    ReportTopic = list_to_binary("lwm2m/"++Epn++"/up/dm"),
    ...
    ReadResult = jsx:encode(#{  <<"imei">> => list_to_binary(Epn),
                                <<"imsi">> => <<"undefined">>,
                                <<"msgType">> => <<"register">>,
                                <<"data">> => #{
                                    <<"alternatePath">> => <<"/">>,
...
case02_update_deregister(Config) ->
    ...
    Register = jsx:encode(#{  <<"imei">> => list_to_binary(Epn),
                              <<"imsi">> => <<"undefined">>,
                              <<"msgType">> => <<"register">>,
                              <<"data">> => #{
                                  <<"alternatePath">> => <<"/">>,
    ...
    {ok,changed} = Method2,
    MsgId2 = RspId2,
    Update = jsx:encode(#{  <<"imei">> => list_to_binary(Epn),
                            <<"imsi">> => <<"undefined">>,
                            <<"msgType">> => <<"update">>,
                            <<"data">> => #{
                                <<"alternatePath">> => <<"/">>,
...
case08_reregister(Config) ->
    ...
    ReadResult = jsx:encode(
                            #{  <<"imei">> => list_to_binary(Epn),
                                <<"imsi">> => <<"undefined">>,
                                <<"msgType">> => <<"register">>,
                                <<"data">> => #{
                                    <<"alternatePath">> => <<"/lwm2m">>,
...
case10_read(Config) ->
    ...
    MsgId1 = 15,
    SubTopic = list_to_binary("lwm2m/"++Epn++"/dn/#"),
    RespTopic = list_to_binary("lwm2m/"++Epn++"/up/dm"),
    ...
    ReadResult = jsx:encode(#{  <<"requestID">> => CmdId, <<"cacheID">> => CmdId,
                                <<"imei">> => list_to_binary(Epn),
                                <<"imsi">> => <<"undefined">>,
                                <<"msgType">> => <<"read">>,
                                <<"data">> => #{
...
case10_read_separate_ack(Config) ->
    ...
    test_send_coap_request( UdpSock,
                            post,
                            sprintf("coap://127.0.0.1:~b/rd?ep=~s&lt=345&lwm2m=1", [?PORT, Epn]),
                            #coap_content{content_format = <<"text/plain">>, payload = <<"</1>, </2>, </3/0>, </4>, </5>">>},
                            [],
                            MsgId1),
    #coap_message{method = Method1} = test_recv_coap_response(UdpSock),
    ?assertEqual({ok,created}, Method1),
...
case11_read_object_tlv(Config) ->
    ...
    Command =   #{
                    <<"requestID">> => CmdId, <<"cacheID">> => CmdId,
                    <<"imei">> => list_to_binary(Epn), <<"imsi">> => <<"undefined">>,
                    <<"msgType">> => <<"read">>,
                    <<"data">> => #{
                        <<"path">> => <<"/3/0">>
                    }
    ...
    test_send_coap_response(UdpSock, "127.0.0.1", ?PORT, {ok, content}, #coap_content{content_format = <<"application/vnd.oma.lwm2m+tlv">>, payload = Tlv}, Request2, false),
    timer:sleep(100),
...
case11_read_object_json(Config) ->
    ...
    test_send_coap_response(UdpSock, "127.0.0.1", ?PORT, {ok, content}, #coap_content{content_format = <<"application/vnd.oma.lwm2m+json">>, payload = Json}, Request2, false),
    timer:sleep(100),
...
case12_read_resource_opaque(Config) ->
    ...
    Opaque = <<20, 21, 22, 23>>,
    test_send_coap_response(UdpSock, "127.0.0.1", ?PORT, {ok, content}, #coap_content{content_format = <<"application/octet-stream">>, payload = Opaque}, Request2, false),
    timer:sleep(100),
...
case20_single_write_4_level_path(Config) ->
...
case60_observe(Config) ->
    ...
    SubTopic = list_to_binary("lwm2m/"++Epn++"/dn/#"),
    RespTopic = list_to_binary("lwm2m/"++Epn++"/up/dm"),
    RespTopicAD = list_to_binary("lwm2m/"++Epn++"/up/ad"),
    emqttc:subscribe(?config(emqttc, Config), RespTopic, qos0),
    ...
    ?assertEqual(ReadResultNotify, test_recv_mqtt_response(RespTopicAD)),
    ...
    CommandJson3 = jsx:encode(Command3),
    ...
    ?assertEqual(ReadResultNotify3, test_recv_mqtt_response(RespTopicAD)).
...
send_read_command_1(CmdId, UdpSock) ->
    Epn = "urn:oma:lwm2m:oma:3",
    CommandTopic = <<"lwm2m/", (list_to_binary(Epn))/binary, "/dn/dm">>,
    Command =   #{
                    <<"requestID">> => CmdId, <<"cacheID">> => CmdId,
