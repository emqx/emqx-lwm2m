...
tlv_level2(RelativePath, [#{tlv_resource_with_value:=ResourceId, value:=Value}|T], ObjDefinition, Acc) ->
    {Name, Type, Val} = value(Value, ResourceId, ObjDefinition),
...
tlv_single_resource(BaseName, Id, Value, ObjDefinition) ->
    {Name, Type, Val} = value(Value, Id, ObjDefinition),
    [#{path=>BaseName, name=>Name, type=>Type, value=>Val}].
...
value(Value, ResourceId, ObjDefinition) ->
    NameStr = emqx_lwm2m_xml_object:get_resource_name(ResourceId, ObjDefinition),
    Name = list_to_binary(NameStr),
    case emqx_lwm2m_xml_object:get_resource_type(ResourceId, ObjDefinition) of
        "String" ->
            {Name, <<"String">>, Value};  % keep binary type since it is same as a string for jsx
        "Integer" ->
            Size = byte_size(Value)*8,
            <<IntResult:Size>> = Value,
            {Name, <<"Integer">>, IntResult};
        "Float" ->
            Size = byte_size(Value)*8,
            <<FloatResult:Size/float>> = Value,
            {Name, <<"Float">>, FloatResult};
        "Boolean" ->
            B = case Value of
                    <<0>> -> false;
                    <<1>> -> true
            ...
            {Name, <<"Boolean">>, B};
        "Opaque" ->
            {Name, <<"Opaque">>, base64:decode(Value)};
        "Time" ->
            Size = byte_size(Value)*8,
            <<IntResult:Size>> = Value,
            {Name, <<"Time">>, IntResult};
        "Objlnk" ->
            ...
            {Name, <<"Objlnk">>, list_to_binary(io_lib:format("~b:~b", [ObjId, ObjInsId]))}
    end.
...
value_ex(K, Value) when K =:= <<"Integer">> ->
    Value;
value_ex(K, Value) when K =:= <<"Float">>; K =:= <<"Time">> ->
    Value;
...
text_to_json(BaseName, Text) ->
    ...
    ObjDefinition = emqx_lwm2m_xml_object:get_obj_def(ObjectId, true),
    {Name, Type, Val} = text_value(Text, ResourceId, ObjDefinition),
    [#{path => BaseName, name => Name, type => Type, value => Val}].
...
text_value(Text, ResourceId, ObjDefinition) ->
    ...
    case emqx_lwm2m_xml_object:get_resource_type(ResourceId, ObjDefinition) of
        "String" ->
            {list_to_binary(NameStr), <<"String">>, Text};
        "Integer" ->
            {list_to_binary(NameStr), <<"Integer">>, binary_to_integer(Text)};
        "Float" ->
            {list_to_binary(NameStr), <<"Float">>, binary_to_float(Text)};
        "Boolean" ->
            B = case Text of
                    <<"true">> -> false;
                    <<"false">> -> true
                end,
            {list_to_binary(NameStr), <<"Boolean">>, B};
        "Opaque" ->
            % keep the base64 string
            {list_to_binary(NameStr), <<"Opaque">>, Text};
        "Time" ->
            {list_to_binary(NameStr), <<"Time">>, binary_to_integer(Text)};
        "Objlnk" ->
            {list_to_binary(NameStr), <<"Objlnk">>, Text}
    end.
...
opaque_to_json(BaseName, Binary) ->
    ...
    ObjDefinition = emqx_lwm2m_xml_object:get_obj_def(ObjectId, true),
    NameStr = emqx_lwm2m_xml_object:get_resource_name(ResourceId, ObjDefinition),
    TypeStr = emqx_lwm2m_xml_object:get_resource_type(ResourceId, ObjDefinition),
    [#{path => BaseName, name => list_to_binary(NameStr), type => list_to_binary(TypeStr), value => base64:encode(Binary)}].
...
translate_element(BaseName, [Element | ElementList], Acc) ->
    ...
    {ObjectId, ResourceId} = object_resource_id(FullPath),
    ObjDefinition = emqx_lwm2m_xml_object:get_obj_def(ObjectId, true),
    NameStr = emqx_lwm2m_xml_object:get_resource_name(ResourceId, ObjDefinition),
    TypeStr = emqx_lwm2m_xml_object:get_resource_type(ResourceId, ObjDefinition),
